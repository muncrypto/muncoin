...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "MunNetwork") type = MunNetwork;
    if(strType == "MunNetworkVariable") type = MunNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MunNetwork) s = "MunNetwork";
    if(type == MunNetworkVariable) s = "MunNetworkVariable";
